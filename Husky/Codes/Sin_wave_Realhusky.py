...
class HuskyController:
    def __init__(self):
        ...
        self.pose_subscriber = rospy.Subscriber('/gazebo/model_states', Odometry, self.update_pose)
    ...
    def move_husky(self):
        ...
        for a in range(len(data)):
            ...
            vel_msg.angular.z = 0
            self.velocity_publisher.publish(vel_msg)
            while round((self.linear_vel(goal_x, goal_y))/0.2, 2) >= 0.1:
                linear_velocity = self.linear_vel(goal_x, goal_y)
                #angular_velocity = self.angular_vel(goal_x, goal_y)
                ...
                self.velocity_publisher.publish(vel_msg)
                data_x.append(self.pose.pose.pose.position.x)
                data_y.append(self.pose.pose.pose.position.y)
                self.rate.sleep()
            vel_msg.linear.x = 0
            self.velocity_publisher.publish(vel_msg)
