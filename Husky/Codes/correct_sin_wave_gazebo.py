...
class HuskyController:
    ...
    def move_husky(self):
        ...
        for a in range(len(data)):
            ...
            vel_msg.angular.z = 0
            self.velocity_publisher.publish(vel_msg)"""
            while round((self.linear_vel(goal_x, goal_y))/0.2, 1) >= 0.1:
                if abs(round((self.angular_vel(goal_x, goal_y))/1.5, 2)) >= 0.01:
                ...
                elif round((self.linear_vel(goal_x, goal_y))/0.2, 2) >= 0.1:
                    linear_velocity = self.linear_vel(goal_x, goal_y)
                    #angular_velocity = self.angular_vel(goal_x, goal_y)
                    ...
                    data_x.append(self.model_pose.position.x)
                    data_y.append(self.model_pose.position.y)
                    self.rate.sleep()
